...
ventana_hasta = 5 # Dias en el que 
lista_opciones = ["1 - Ver temperatura más Alta", "2 - Ver temperatura más Baja", "3 - Ver temperatura media", "4 - Ver dias con subidas de 5 grados"] # Cuando imprimo, imprimo por la posicion de esta lista+1
...
def calculo_temp_dias(lista_grados):
    lista_grados_aux = []
    for i in range(0, len(lista_grados), ventana_hasta):
        i = 0
        a = ventana_hasta
        z = 1
        lista_grados_aux = lista_grados.copy()


        
        if abs(diferencia) > 5:
            entre_dia_y_dia.append(i+1)
            entre_dia_y_dia.append(i+(ventana_hasta-1))
...
def imprimicion_final(numero_lista_opciones):
    if numero_lista_opciones == 1:
        ...
        print("La opcion introducido no existe")
